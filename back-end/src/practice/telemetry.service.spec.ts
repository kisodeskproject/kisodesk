...
describe('TelemetryService', () => {
  ...
  it('recalcula métricas y conserva errores aunque haya Backspace', () => {
    ...
    expect(result.bigrams.get('a\u0000a')?.presses).toBe(1);
  ...
  it('excluye pausas y latencias extremas sin truncarlas', () => {
    ...
    expect(result.bigrams.get('b\u0000c')?.latencySamples).toBe(0);
  });
  ...
  it('clasifica reemplazos, Backspace consecutivos y Backspace vacío', () => {
    const result = service.derive({ version: 1, text: 'ab', startedAt: 0, pausedMs: 0, events: [
      ...
      { sequence: 2, kind: 'input', timestamp: 200, code: 'KeyA', key: 'y', position: 0, expected: 'a', typed: 'y', correct: false },
      { sequence: 3, kind: 'input', timestamp: 300, code: 'KeyA', key: 'a', position: 0, expected: 'a', typed: 'a', correct: true },
      { sequence: 4, kind: 'backspace', timestamp: 400, code: 'Backspace', key: 'Backspace', position: 1 },
      { sequence: 5, kind: 'backspace', timestamp: 500, code: 'Backspace', key: 'Backspace', position: 0 },
      { sequence: 6, kind: 'input', timestamp: 600, code: 'KeyB', key: 'z', position: 1, expected: 'b', typed: 'z', correct: false },
    ]});
    expect(result.correctedErrors).toBe(2);
    expect(result.uncorrectedErrors).toBe(1);
    expect(result.backspaces).toBe(3);
