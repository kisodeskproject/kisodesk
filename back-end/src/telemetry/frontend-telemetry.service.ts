...
export class FrontendTelemetryService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    @InjectMetric('typing_frontend_initializations_sampled_total') private readonly initializations: Counter,
    @InjectMetric('typing_frontend_page_views_observed_total') private readonly pageViews: Counter,
    @InjectMetric('typing_anonymous_sessions_observed_total') private readonly anonymousSessions: Counter,
    @InjectMetric('typing_practice_started_observed_total') private readonly practiceStarted: Counter,
    @InjectMetric('typing_practice_completed_observed_total') private readonly practiceCompleted: Counter,
    @InjectMetric('typing_practice_abandoned_observed_total') private readonly practiceAbandoned: Counter,
  ) {
  ...
  onModuleInit() {
    ...
    this.cleanupTimer = setInterval(() => void this.removeExpiredDailyAnalytics(), 24 * 60 * 60 * 1000);
  ...
  private recordEvent(event: FrontendTelemetryEventDto) {
    ...
    if (event.type === 'session') {
      ...
      this.initializations.labels(this.frontendVersion).inc();
      return;
    }
    ...
    if (event.type === 'practice_started' || event.type === 'practice_completed' || event.type === 'practice_abandoned') {
      if (!event.authState || !event.language || !event.layout || !SUPPORTED_LAYOUT_IDS.has(event.layout)) return;
      const metric = event.type === 'practice_started'
        ? this.practiceStarted
        : event.type === 'practice_completed' ? this.practiceCompleted : this.practiceAbandoned;
